...
`twt/captcha` package and setting the `minimum-stability` to `dev` (required for Laravel 5) in your
...
```
composer require twt/captcha
```

Update your packages with ```composer update``` or install with ```composer install```.

In Windows, you'll need to include the GD2 DLL `php_gd2.dll` in php.ini. And you also need include `php_fileinfo.dll` and `php_mbstring.dll` to fit the requirements of `twt/captcha`'s dependencies.
...
```php
    'providers' => [
        // ...
        'TwT\Captcha\CaptchaServiceProvider',
    ]
```
for Laravel 5.1+
```php
    'providers' => [
        // ...
        TwT\Captcha\CaptchaServiceProvider::class,
    ]
```
...
```php
    'aliases' => [
        // ...
        'Captcha' => 'TwT\Captcha\Facades\Captcha',
    ]
```
for Laravel 5.1+
```php
    'aliases' => [
        // ...
        'Captcha' => TwT\Captcha\Facades\Captcha::class,
    ]
```
...
* [L5 Captcha on Packagist](https://packagist.org/packages/twt/captcha)
